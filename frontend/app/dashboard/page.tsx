...
export default function DashboardPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      {error && (
        <ErrorMessage message="Αποτυχία φόρτωσης δεδομένων." />
      )}

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <Link
          href="/announcements"
          className="p-4 rounded-xl shadow bg-white dark:bg-gray-800 hover:shadow-md transition block"
        >
          <h2 className="text-lg font-semibold mb-2">📢 Ανακοινώσεις</h2>
          <p className="text-sm text-gray-600 dark:text-gray-300">
            {announcements.length} ανακοινώσεις
          </p>
        </Link>
              ...
              <PieChart>
                <Pie
                  ...
                  data={[
                    {
                      name: 'Ανοιχτά',
                      value: filteredRequests.filter((r) => r.status === 'open').length,
                    },
                    {
                      name: 'Σε εξέλιξη',
          ...
          <ul className="space-y-2">
            {topRequests.map((r) => (
              <li key={r.id}>
                <Link
                  href={`/requests/${r.id}`}
                  className="block p-3 border rounded-lg shadow-sm hover:shadow transition bg-white"
                >
            ...
            {announcements
              ...
              .map((a) => (
                <li key={a.id}>
                  <Link
                    href={`/announcements`}
                    className="block p-4 border rounded-lg shadow-sm hover:shadow transition bg-white dark:bg-gray-800"
                  >
                    <h3 className="font-semibold text-blue-900 dark:text-white mb-1 line-clamp-2">
                      {a.title}
                    </h3>
                    <p className="text-xs text-gray-500 mb-1">
                      Έναρξη: {new Date(a.start_date).toLocaleDateString('el-GR')}
                    </p>
